...
type PanelTab = 'members' | 'history'
type HistoryFilter = 'all' | 'image' | 'file' | 'link'

const activeTab = ref<PanelTab>('members')
...
async function loadMoreSearchResults() {
  ...
  const nextPage = historySearchPage.value + 1
  historySearching.value = true
  try {
    const res = await searchMessages(props.conversationId, historySearchQuery.value.trim(), nextPage, 20)
    if (res.code === 200) {
      historySearchResults.value = [...historySearchResults.value, ...(res.data.items || [])]
      historySearchPage.value = nextPage
      historySearchTotal.value = res.data.total
    }
  } catch {
    // silent
...
const memberSorted = computed(() => {
  ...
  return [...detail.value.members].sort((a, b) => {
...
function handleLeave() {
  const name = detail.value?.name || '此群聊'
  if (confirm(`确定要退出${name}吗？`)) {
...
async function handleAvatarChange(e: Event) {
  ...
  try {
    const res = await updateGroupAvatar(props.conversationId, file)
    if (res.code === 200 && detail.value) {
      detail.value.avatar = res.data as unknown as string
      // 同步更新 store 中的会话列表
      const conv = store.conversations.find(c => c.id === props.conversationId)
      if (conv) conv.avatar = detail.value.avatar
    }
  } catch {
    // silent
  } finally {
    ...
    >
      <div class="absolute right-0 top-0 h-full w-[360px] bg-vscode-sidebar border-l border-vscode-border shadow-2xl flex flex-col">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-vscode-border">
          <h3 class="text-sm font-semibold text-vscode-text">群聊信息</h3>
          <button
        ...
        <template v-else-if="detail">
          ...
          <div class="px-4 py-4 border-b border-vscode-border">
            <div class="flex items-center gap-4 mb-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-vscode-text truncate">{{ detail.name || '未命名群聊' }}</div>
                <div class="text-xs text-vscode-text-secondary mt-0.5">
                  群号：{{ detail.groupCode || '-' }}
                </div>
                <div class="text-xs text-vscode-text-secondary">
                  {{ detail.memberCount ?? detail.members?.length ?? 0 }} 名成员
                </div>
              </div>
            </div>
          ...
          <!-- 成员列表 -->
          <div v-if="activeTab === 'members'" class="flex-1 overflow-y-auto">
            <div
              v-for="member in memberSorted"
              :key="member.userId"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm text-vscode-text truncate">{{ member.nickname }}</div>
              </div>
              <span class="px-2 py-0.5 text-xs rounded-full" :class="getRoleClass(member.role)">
                <Crown v-if="member.role === 'OWNER'" class="w-3 h-3 inline mr-0.5" />
                <Shield v-else-if="member.role === 'ADMIN'" class="w-3 h-3 inline mr-0.5" />
