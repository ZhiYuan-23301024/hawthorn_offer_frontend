...
<template>
  ...
  <div v-if="message.messageType === 'SYSTEM'" class="flex justify-center py-2">
    <span class="text-xs text-vscode-text-secondary bg-vscode-sidebar px-3 py-1 rounded-full">
      {{ message.content }}
    </span>
  </div>
  ...
  <div v-else class="flex px-4 py-1.5" :class="isOwn ? 'flex-row-reverse' : 'flex-row'">
    ...
    <div class="flex-shrink-0" :class="isOwn ? 'ml-2' : 'mr-2'">
      <div
        v-if="showSender"
        class="w-8 h-8 rounded-full bg-vscode-active flex items-center justify-center text-xs text-vscode-text overflow-hidden"
      >
        <img
    ...
    <div class="max-w-[65%]" :class="isOwn ? 'items-end' : 'items-start'">
      ...
      <div
        v-if="showSender"
        class="text-xs text-vscode-text-secondary mb-0.5"
      ...
      <div class="flex flex-col" :class="isOwn ? 'items-end' : 'items-start'">
        ...
        <div
          ...
          :class="isOwn
            ? 'bg-vscode-info/20 text-vscode-text'
        ...
        >
          <template v-for="(seg, i) in textSegments" :key="i">
            <a
              ...
              :class="isOwn ? 'text-blue-300' : 'text-blue-400'"
              @click.stop
            >{{ seg.value }}</a>
        ...
        <div v-else-if="message.messageType === 'IMAGE'" class="max-w-[240px]">
          <template v-if="message.attachments && message.attachments.length > 0">
            <img
              :src="getAttachmentUrl(message.attachments[0].storagePath)"
              :alt="message.attachments[0].fileName"
              class="rounded-lg cursor-pointer max-h-[200px] object-cover hover:opacity-90 transition-opacity"
              @click="showFullImage = true"
            />
          </template>
          <div v-else class="px-3 py-2 rounded-lg text-sm bg-vscode-active text-vscode-text-secondary">
            [图片]
          </div>
        </div>
        ...
        <template v-else-if="message.messageType === 'FILE'">
          <a
            ...
            :href="getAttachmentUrl(message.attachments[0].storagePath)"
            :download="message.attachments[0].fileName"
            target="_blank"
            class="px-3 py-2 rounded-lg text-sm bg-vscode-active flex items-center gap-2 hover:bg-vscode-selected transition-colors cursor-pointer"
            style="text-decoration: none; color: inherit;"
            @click.stop
          >
            <span class="text-vscode-text-secondary">📎</span>
            <span class="text-vscode-text">{{ message.attachments[0].fileName }}</span>
            <span class="text-vscode-text-secondary text-xs">({{ (message.attachments[0].fileSize / 1024).toFixed(1) }} KB)</span>
          </a>
          <div v-else class="px-3 py-2 rounded-lg text-sm bg-vscode-active text-vscode-text-secondary">
            [文件]
          </div>
        ...
        <!-- 时间 -->
        <span class="text-xs text-vscode-text-secondary mt-0.5 px-1">
  ...
  <Teleport to="body">
    <div
      v-if="showFullImage && message.attachments && message.attachments.length > 0"
